  ...
  try {
    ...
    if (!data.present) {
      await createNotification(
        ...
        "Attendance Alert",
        `${student.name} ${student.surname} was marked absent ${data.lessonId ? "for a lesson" : "today"}${student.class ? ` in ${student.class.name}` : ""}. ${data.note ? `Note: ${data.note}` : ""}`,
        "ATTENDANCE",
        {
          studentId: data.studentId,
      ...
      const attendancePromises = data.attendanceRecords.map(async (record) => {
        ...
        if (!record.present) {
          await createNotification(
            student.parent.user.id,
            "Attendance Alert",
            `${student.name} ${student.surname} was marked absent ${data.lessonId ? "for a lesson" : "today"}${student.class ? ` in ${student.class.name}` : ""}. ${record.note ? `Note: ${record.note}` : ""}`,
            "ATTENDANCE",
            {
              studentId: record.studentId,
              attendanceId: attendance.id,
...
export const getStudentAttendance = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    logger.info("Student attendance retrieved", {
      userId: req.user?.id,
      studentId,
    ...
    handleError(res, error, "Failed to retrieve student attendance")
  }
}
...
export const getClassAttendance = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    logger.info("Class attendance retrieved", {
      userId: req.user?.id,
      classId,
      date: attendanceDate.toISOString(),
    ...
    handleError(res, error, "Failed to retrieve class attendance")
  }
}
...
export const getAttendanceAnalytics = async (req: AuthRequest, res: Response) => {
  try {
    ...
    handleError(res, error, "Failed to retrieve attendance analytics")
  }
}
